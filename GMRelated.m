%灰色关联分析步骤
%【1】确定比较对象（评价对象）(就是数据，并且需要进行规范化处理
% ，就是标准化处理，见下面例题的表格数据)和参考数列
% （评价标准，一般该列数列都是1，就是最优的的情况） 
%【2】确定各个指标权重，可用层次分析确定 
%【3】计算灰色关联系数 
%【4】计算灰色加权关联度 
%【5】评价分析
...
x1=[1.14 1.49 1.69 2.12 2.43 4.32 5.92 6.07 7.85;
    ...
    4.87 5.89 6.76 7.97 8.84 10.05 11.31 12.25 11.64];%原始数据5行9列，每一行是一个对象
x=x1;
for i=1:5
  for j=1:9
  x(i,j)=x(i,j)/x1(1,j);
